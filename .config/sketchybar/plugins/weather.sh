...
API_KEY="205ed87340b44887b8183332241903"
...
render_item() {
	if [ "$CITY" = "" ]; then
		...
		args+=(--set $NAME icon="$ICON" icon.font="Hack Nerd Font:Bold:14.0" label="${TEMP_F}°F" label.drawing=on)
	fi
...
update() {
	...
	if [ "$CITY" != "" ]; then
		...
		TEMP_C=$(echo $DATA | jq -r '.current.temp_c | floor')
